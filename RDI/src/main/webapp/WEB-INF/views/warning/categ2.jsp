...
function to_page(pn){
	...
	{
		$.ajax({
			url:"${APP_PATH}/warning/allExceptionDataMiddle",
			data:{"pn":pn,
				  "categName":categName,
				  "rise":rise,
				  "decrease":decrease,	
				  "year":year,	
				  "month":month,
				  "classifybymiddleId":classifybymiddleId,
				},
			type:"POST",
			success:function(result){
				console.log(result);
	...
	{
		$.ajax({
			url:"${APP_PATH}/warning/withDisExceptionDataMiddle",
			data:{"pn":pn,
				  "categName":categName,
				  "rise":rise,
				  "decrease":decrease,	
		...
		{
		$.ajax({
			url:"${APP_PATH}/warning/withPharamExceptionDataMiddle",
			data:{"pn":pn,
				  "categName":categName,
				  "rise":rise,
				  "decrease":decrease,	
...
function build_warning_table(result){
	...
	$.each(warning,function(index,item){
		...
		var cityCategIndex=$("<td></td>").append(item.categ2Index.categIndex);
		var categIndex=$("<td></td>").append(item.categIndex);
		var percent=$("<td></td>").append((((item.categIndex-item.categ2Index.categIndex)/item.categ2Index.categIndex)*100).toFixed(2)+"%");
