...
<div class="container">
    ...
    <div class="row">
        ...
        <h6 style="color:red;">注2：解释下浮百分比 和上浮百分比的含义。</h6>
        <h6 style="color:red;">注3：地区和药店不是必选项，若都不选择，则统计的是整个宿迁市的价格指数；若仅选择地区未选择药店，则统计的是该地区的价格指数.</h6>
    </div>
	    	...
	    	<table class="table table-bordered table-hover" id="warning_table" >
			   <thead>
			       <tr class='head'>
		    	       ...
		    	       <th style="text-align:center">标准限定日费用</th>     
...
function to_page(pn){
	...
	{
		$.ajax({
			url:"${APP_PATH}/warning/allExceptionDataLarge",
			data:{"pn":pn,
				  "categName":categName,
				  "rise":rise,
				  "decrease":decrease,	
				  "year":year,	
				  "month":month,
				  "classifybylargeId":classifybylargeId,
	...
	{
		$.ajax({
			url:"${APP_PATH}/warning/withDisExceptionDataLarge",
			data:{"pn":pn,
				  "categName":categName,
				  "rise":rise,
				  "decrease":decrease,	
		...
		{
		$.ajax({
			url:"${APP_PATH}/warning/withPharamExceptionDataLarge",
			data:{"pn":pn,
				  "categName":categName,
				  "rise":rise,
				  "decrease":decrease,	
...
function build_warning_table(result){
	...
	$.each(warning,function(index,item){
		...
		var cityCategIndex=$("<td></td>").append(item.categ1Index.categIndex);
		var categIndex=$("<td></td>").append(item.categIndex);
		var percent=$("<td></td>").append((((item.categIndex-item.categ1Index.categIndex)/item.categ1Index.categIndex)*100).toFixed(2)+"%");
