...
$(function(){
	$("#classifybysmall").change(function(){
		...
		if(classifybysmall!=""){
			$.ajax({
				url:"${APP_PATH}/category/getClassifyByKinds",
...
function getClassify(ele){
	//清空之前下拉列表的值
	$("ele option:not(:first)").remove();
	$.ajax({
		url:"${APP_PATH}/category/getClassify",
		type:"GET",
		success:function(result){
			console.log(result);
			$.each(result.extend.classify,function(){
				var optionEle = $("<option></option>").append(this.categName).attr("value",this.classify);
				optionEle.appendTo(ele);
		});
		}
	});
...
function to_page(pn){
	...
	{
		$.ajax({
			url:"${APP_PATH}/warning/allExceptionData",
			data:{"pn":pn,
				  "categName":categName,
				  "rise":rise,
				  "decrease":decrease,	
				  "year":year,	
				  "month":month,
				  "classifybykindsId":classifybykindsId,
				},
			type:"POST",
			success:function(result){
				console.log(result);
	...
	{
		$.ajax({
			url:"${APP_PATH}/warning/withDisExceptionData",
			data:{"pn":pn,
				  "categName":categName,
				  "rise":rise,
				  "decrease":decrease,	
		...
		{
		$.ajax({
			url:"${APP_PATH}/warning/withPharamExceptionData",
			data:{"pn":pn,
				  "categName":categName,
				  "rise":rise,
				  "decrease":decrease,	
...
function build_warning_table(result){
	...
	$.each(warning,function(index,item){
		...
		var cityCategIndex=$("<td></td>").append(item.categ4Index.categIndex);
		var categIndex=$("<td></td>").append(item.categIndex);
		var percent=$("<td></td>").append((((item.categIndex-item.categ4Index.categIndex)/item.categ4Index.categIndex)*100).toFixed(2)+"%");
...
if(districtId==null||districtId=="" || $.trim(districtId)==""){
	$.ajax({
		url:"${APP_PATH}/warning/horizCmpDrugIndex4",
		data:{"pn":pn,
			  "categName":categName,
			  "year":year,	
			  "month":month,
		...
		success:function(result){
			...
			build_page_nav(result);
				...
				else if(drugstoreId==null||drugstoreId=="" || $.trim(drugstoreId)=="")
				{
					...
					$.ajax({
						url:"${APP_PATH}/warning/getCompareIndex",
						data:{"pn":pn,
							  "categName":categName,
							  "year":year,	
							  "month":month,
