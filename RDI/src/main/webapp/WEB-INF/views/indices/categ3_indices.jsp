...
<script type="text/javascript">
		...
		function to_page(pn){
			...
			if(districtId==null||districtId=="" || $.trim(districtId)==""){
				$.ajax({
					url:"${APP_PATH}/categ3index/getCateg3Indices",
					data:{"pn":pn,
						  "categName":categName,
						  "indexSort":indexSort,
				...
				{
				$.ajax({
					url:"${APP_PATH}/categ3index/getCateg3IndicesWithDistrict",
					data:{"pn":pn,
						  "categName":categName,
						  "indexSort":indexSort,
						  "statisYear":statisYear,
				...
				$.ajax({
					url:"${APP_PATH}/categ3index/getCateg3IndicesWithPharm",
					data:{"pn":pn,
						  "categName":categName,
						  "indexSort":indexSort,
						  "statisYear":statisYear,
		...
		function build_all_city_index_table(result){
			...
			var categ3indices = result.extend.pageInfo.list;
			$.each(categ3indices,function(index,item){
				
			
				var categNameTd = $("<td></td>").append(item.category.categName);
				var levelTd = $("<td></td>").append("小类");
		...
		$(document).on("click",".vertic_cmp_btn",function(){
			...
			if(pharmId!=null && pharmId!="" && pharmId.trim()!="")
				{
			    	url ="${APP_PATH}/categ3index/toCateg3IndicesDetail?categId="+categId+"&pharmId="+pharmId;
				
				}
			else if(districtId!=null && districtId!="" && districtId.trim()!="")
				{
				    url ="${APP_PATH}/categ3index/toCateg3IndicesDetail?categId="+categId+"&districtId="+districtId;	
				}
			else
				url="${APP_PATH}/categ3index/toCateg3IndicesDetail?categId="+categId;
		...
		$(document).on("click",".horiz_cmp_btn",function(){
			...
			url="${APP_PATH}/categ3index/toHorizCmpDetail?categId="+categId+"&year="+year+"&month="+month;
