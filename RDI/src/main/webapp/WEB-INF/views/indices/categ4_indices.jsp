...
<div class="container">
    ...
    <div class="row">
        ...
        <h6 style="color:red;">注3：下表中，点击"指数对比"下方的"横向"按钮可以查出该类别在不同的统计范围下的指数，点击"纵向"按钮可查看该类别最近一年的指数变化情况.</h6>
...
<script type="text/javascript">
		...
		function to_page(pn){
			...
			if(districtId==null||districtId=="" || $.trim(districtId)==""){
				$.ajax({
					url:"${APP_PATH}/categ4index/getCateg4Indices",
					data:{"pn":pn,
						  "categName":categName,
						  "indexSort":indexSort,
				...
				{
				$.ajax({
					url:"${APP_PATH}/categ4index/getCateg4IndicesWithDistrict",
					data:{"pn":pn,
						  "categName":categName,
						  "indexSort":indexSort,
						  "statisYear":statisYear,
				...
				$.ajax({
					url:"${APP_PATH}/categ4index/getCateg4IndicesWithPharm",
					data:{"pn":pn,
						  "categName":categName,
						  "indexSort":indexSort,
						  "statisYear":statisYear,
		...
		function build_all_city_index_table(result){
			...
			var categ4indices = result.extend.pageInfo.list;
			$.each(categ4indices,function(index,item){
				
			
				var categNameTd = $("<td></td>").append(item.category.categName);
				var levelTd = $("<td></td>").append("代表品类");
		...
		$(document).on("click",".vertic_cmp_btn",function(){
			...
			if(pharmId!=null && pharmId!="" && pharmId.trim()!="")
				{
			    	url ="${APP_PATH}/categ4index/toCateg4IndicesDetail?categId="+categId+"&pharmId="+pharmId;
				
				}
			else if(districtId!=null && districtId!="" && districtId.trim()!="")
				{
				    url ="${APP_PATH}/categ4index/toCateg4IndicesDetail?categId="+categId+"&districtId="+districtId;	
				}
			else
				url="${APP_PATH}/categ4index/toCateg4IndicesDetail?categId="+categId;
		...
		$(document).on("click",".horiz_cmp_btn",function(){
			...
			url="${APP_PATH}/categ4index/toHorizCmpDetail?categId="+categId+"&year="+year+"&month="+month;
