...
<div class="container">
      <div class="row Top">
	        ...
	        <div class="col-lg-3 col-md-4 col-sm-5  col-xs-5">
	                           药店：<select class="drugstore" name="drugstore" id="drugstore">
    ...
    <div class="row">
        ...
        <h6 style="color:red;">注2：药品名称可不填，此时将查询所有药品的价格指数.</h6>
        <h6 style="color:red;">注3：下表中，点击"指数对比"下方的"横向"按钮可以查出该药品在不同的统计范围下的指数，
                                                                                    点击"纵向"按钮可查看该药品最近一年的指数变化情况.</h6>
    </div>
	    	...
	    	<table class="table table-bordered table-hover" id="drugindex_table" >
			   <thead>
			       <tr class='head'>
		    	       ...
		    	       <th id="table-avg-price" style="text-align:center">平均价格</th> 	      
...
<script type="text/javascript">
		...
		//1、页面加载完成以后，直接去发送ajax请求,要到地区和药店信息
		$(function(){
		...
		function to_page(pn){
			...
			var pharmId = $(".drugstore[name='drugstore']").val();
			...
			if(districtId==null||districtId=="" || $.trim(districtId)==""){
			...
			else if(pharmId==null||pharmId=="" || $.trim(pharmId)=="")
				{
				$.ajax({
					url:"${APP_PATH}/drugindex/getDrugIndicesWithDistrict",
					data:{"pn":pn,
						  "drugName":drugName,
						  "indexSort":indexSort,
						  "statisYear":statisYear,
				...
				$.ajax({
					url:"${APP_PATH}/drugindex/getDrugIndicesWithPharm",
					data:{"pn":pn,
						  "drugName":drugName,
						  "indexSort":indexSort,
						  "statisYear":statisYear,
		...
		function build_all_city_index_table(result){
			...
			$.each(drugs,function(index,item){
	            ...
	            horizCmpBtn.attr("details-drug-id",item.drugId);
	           ...
	           //为纵向比较按钮添加自定义的属性，来表示当前药品id和统计日期
		...
		$(document).on("click",".horiz_cmp_btn",function(){
			...
			url="${APP_PATH}/drugindex/toHorizCmpDetail?drugId="+drugId+"&year="+year+"&month="+month;
