...
//绘制整个市的某类药品指数变化折线图
  function showLine(categId){
  	...
  	$.ajax({
  		url:"${APP_PATH}/categ0index/getRecentCateg0IndicesById",
  		...
  		success:function(result){
		   ...
		   for(var i=0;i<ydata2.data.length;i++){
  		 	...
  		 	buildLineNew('line-labels-upper-left',[ydata],xdata,'总指数','定基指数变化趋势');
  		 	buildLineNew('line-labels-upper-right',[ydata1],xdata,'总指数','环比指数变化趋势');
  		 	
  		 	buildLineNew('line-labels-lower-left',[ydata2],xdata,'总指数','同比指数变化趋势');
  		// buildLineNew('line-labels-lower-right',[ydata],xdata,'总指数','指数变化趋势');
  		}
...
//绘制某个区域的某类药品指数变化折线图
  function showLineWithDistrict(categId,districtId){
  	...
  	$.ajax({
  		url:"${APP_PATH}/categ0index/getRecentCateg0IndicesWithDistrictById",
...
function showLineWithPharm(categId,pharmId){
	...
	$.ajax({
		url:"${APP_PATH}/categ0index/getRecentCateg0IndicesWithPharmById",
