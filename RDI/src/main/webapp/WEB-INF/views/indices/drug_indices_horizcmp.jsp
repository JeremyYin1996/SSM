...
<div class="container">
  <div class="row Top">
    ...
    <div class="row">
        <h5 style="color:red;">注1：若统计范围为"宿迁市"，则表示整个宿迁市的药品指数；若统计范围为"市区"、"沭阳县"、"泗阳县"或"泗洪县"表示各县区的药品指数。</h5>
        <h5 style="color:red;">注2：点击"纵向比较"按钮可以查看最近一年指数变化情况。</h5>
    </div>
    <div class="row Center">
	    	<table class="table table-bordered table-hover" id="drugindex_table" >
			   <thead>
			       <tr class='head'>
		    	     ...
		    	     <!--    <th id="table-pharm-name" style="text-align:center">药店名称</th>-->
		    	       ...
		    	       <th style="text-align:center">纵向比较</th>   
			      </tr>
			   </thead>		
		     <tbody>
...
<script type="text/javascript">
   ...
   $(function(){
	...
	$.ajax({
		url:"${APP_PATH}/drugindex/horizCmpDrugIndex",
		data:{"drugId":drugId,
			  "year":year,
			  "month":month,
			},
   ...
   function build_drug_index_table(result){
		$("#drugindex_table tbody").empty();		

		var drug = result.extend.drug;		
		var drugindex= result.extend.drugIndex;
		$.each(drugindex,function(index,item){
		
			var drugNameTd = $("<td></td>").append(drug.drugName);
	    	var approvalNumberTd = $("<td></td>").append(drug.approvalNumber);
			var dosageFormTd = $("<td></td>").append(drug.dosageForm);
			var specTd = $("<td></td>").append(drug.spec);	
			var manufacturerTd = $("<td></td>").append(drug.manufacturer);
			var districtTd = $("<td></td>").append("宿迁市");
			//var pharmacyTd=$("<td></td>").append(" ");
			var avgpriceTd=$("<td></td>").append(item.avgPrice);
           ...
           //为纵向比较按钮添加自定义的属性，来表示当前分类id和统计日期
            vertiCmpBtn.attr("details-drug-id",item.drugId);
			...
			$("<tr></tr>").append(drugNameTd)				
				...
				.append(statisDateTd)
				.append(btnTd)
		 		.appendTo("#drugindex_table tbody");
		});
		
		
		drugIndexWithDis=result.extend.drugIndexWithDis;
		$.each(drugIndexWithDis,function(index,item){
		...
		drugIndexWithPharm=result.extend.drugIndexWithPharm;
		$.each(drugIndexWithPharm,function(index,item){
			...
			//var pharmacyTd=$("<td></td>").append(item.pharmacy.pharmName);
			var avgpriceTd=$("<td></td>").append(item.avgPrice);
			//var avgpriceTd=$("<td></td>").append(" ");
			var drugIndexTd = $("<td></td>").append(item.drugIndex);
			var statisDateTd = $("<td></td>").append(item.statisDate);
   ...
   $(document).on("click",".vertic_cmp_btn",function(){
		...
		var drugId=$(this).attr("details-drug-id");	
		...
		if(pharmId!=null && pharmId!="" && pharmId.trim()!="")
			{
		    	url ="${APP_PATH}/drugindex/toVerticCmpDetail?drugId="+drugId+"&pharmId="+pharmId;
			
			}
		else if(districtId!=null && districtId!="" && districtId.trim()!="")
			{
			    url ="${APP_PATH}/drugindex/toVerticCmpDetail?drugId="+drugId+"&districtId="+districtId;	
			}
		else
			url="${APP_PATH}/drugindex/toVerticCmpDetail?drugId="+drugId;
